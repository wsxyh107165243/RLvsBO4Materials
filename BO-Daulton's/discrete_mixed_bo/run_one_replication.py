...
def ignore_all_warnings(func):
    ...
    def wrapper(*args, **kwargs):
        with warnings.catch_warnings():
            warnings.simplefilter("ignore", category = NumericalWarning)  # 忽略所有警告
...
def run_one_replication(
    ...
    if X_init is None:
        if 'discrete' in function_name:
            ''' self-impl discrete ramdom sampling '''
            _dis_val_count = (base_function.integer_bounds.max() + 1).int().item()
            _x_low, _x_high = base_function.bounds.min().item(), base_function.bounds.max().item()
            _dis_vals = np.linspace(_x_low, _x_high, _dis_val_count).round(8)
            _x_orig = torch.tensor(
                np.random.RandomState(seed).choice(_dis_vals, size = (n_initial_points, base_function.dim))
            )
            X = (_x_orig - _x_low) / (_x_high - _x_low)
            Y = eval_problem(X, base_function=base_function)
    ...
    for i in range(existing_iterations, iterations):
        ...
        if label == "sobol":
            ...
            # torch.cuda.empty_cache()    # not sure
            # free memory

...
from botorch.test_functions.synthetic import Hartmann, Rosenbrock, SyntheticTestFunction
...
class Rastrigin(SyntheticTestFunction):
    ...
    d-dimensional function (evaluated on `[-5.0, 5.0]^d`)

    f has one minimizer for its global minimum at `z_1 = (0, 0, ..., 0)` with
    `f(z_1) = 0`.

    NOTE: We do not use botorch's Rastrigin function. We implement
        our own function and define our own 
    """
    ...
    def evaluate_true(self, X: Tensor) -> Tensor:
        a, d = self.a, self.dim
        res = a * d + torch.sum(X ** 2 - a * torch.cos(2 * torch.pi * X), dim = -1)
        return res
...
class Ackley(SyntheticTestFunction):
    ...
    def evaluate_true(self, X: Tensor) -> Tensor:
        sum_sq_term = -self.a * torch.exp(-self.b * torch.sqrt(torch.sum(X ** 2, dim=-1) / self.dim))
        cos_term = -torch.exp(torch.sum(torch.cos(self.c * X), dim=-1) / self.dim)
        return self.a + torch.exp(torch.tensor(1.0)) + sum_sq_term + cos_term
    ...
    **kwargs,
    ...
    if batch_size >= 2:
        if label[-2:] == "ei":
            ...
            acq_func = qExpectedImprovement(model, best_f = train_Y.max(), sampler = sampler)
    if "pr" in label:
...
def get_problem(name: str, dim: Optional[int] = None, **kwargs) -> DiscreteTestProblem:
    ...
    if name == "discrete_hartmann":
        ...
        integer_bounds[1, :dim] = 50  # 10 values
    ...
    elif name == 'discrete_rastrigin':
        # dim = 10
        integer_bounds = torch.zeros(2, dim)
        integer_bounds[1, :dim] = 50  # 10 values
        rast = Rastrigin(dim = dim, negate=True)
        return DiscretizedBotorchTestProblem(
            problem = rast,
            integer_indices = list(range(dim)),
            integer_bounds = integer_bounds,
